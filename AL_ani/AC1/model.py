...
def scale_cell(cell,scaling_factor):
  #print(cell.dtype,scaling_factor.dtype)
...
class Model:
    def __init__(self,nmodels):
        ...
        try:
            ...
            path=os.getcwd()
        sae_file = os.path.join(path, 'sae_linfit_zero.dat')
        const_file  = os.path.join(path, 'rC.params')
        consts = torchani.neurochem.Constants(const_file)
        self.aev_computer = torchani.AEVComputer(**consts).to(device)
        self.energy_shifter = torchani.neurochem.load_sae(sae_file).to(device)
        ...
        for i in range(0,self.nmodels):
            ...
            tmp.load_state_dict(torch.load(fname,map_location=device))            
            tmodel = torchani.nn.Sequential(self.aev_computer,tmp,self.energy_shifter).to(device).to(torch.float64)
            nns.append(tmp)
    ...
    def gvalues(self,species,coordinates,cell,pbc):
        ...
        for nnp in self.models:
            displacement = torch.zeros(3,3,requires_grad=True,dtype=torch.float32).to(device)
            scaling_factor = torch.eye(3,dtype=torch.float32).to(device)+displacement
            cell.to(device)
            scoordinates=scaled_coordinates(cell,coordinates,pbc).to(device)
    ...
    def lvalues(self,species,coordinates,cell,pbc,mylist):
        ...
        for nn in self.nns:
            displacement = torch.zeros(3,3,requires_grad=True,dtype=torch.float64).to(device)
            scaling_factor = torch.eye(3,dtype=torch.float64).to(device)+displacement
            cell.to(device)
            ...
            loclist=torch.tensor(tlist,dtype=torch.int64,device=species.device) #for lammps
            local_energies=torch.index_select(atomic_energies,1,loclist)#for lammps
    ...
    def atomic(self,species,coordinates,cell,pbc):
        tlenergy=[]
        tenergy=[]
        tforces=[]
        tstress=[]
        ...
        for nn in self.nns:
            ...
            local_energies=atomic_energies
