...
def EvalMol(filehead,mname,datanum):
    ...
    fname = filehead+"_sae_"+mname+".dat"
...
for i in range(0,nensem):
    ...
    dataloc = 'train'+str(i)+"/"+dataname
    modelf='best'+str(i)
    maee,maef=EvalMol(dataloc,modelf,1000)
    out=""
    out+=str(maee)+" "+str(maef)+"\n"
    listmae.append(maef)
    logf = "eval"+str(i)+".dat"
    f=open(logf,'w')
    f.write(out)
...
minv = min(listmae)
min_index = listmae.index(minv)

print(min_index)
com="cp best"+str(min_index)+".pt bestbest.pt"
os.system(com)
