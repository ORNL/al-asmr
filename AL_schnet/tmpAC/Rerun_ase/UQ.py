...
def UQatomicMol(filehead,nmodels,datanum):
    ...
    nref_en = hartree2kcalmol(ref_en)
    nexp_en = hartree2kcalmol(exp_en +diff_mean) #+ regr.intercept_
    #nexp_en = hartree2kcalmol(exp_en +diff_min) #+ regr.intercept_
    #nexp_en = hartree2kcalmol(exp_en) #+ regr.intercept_
    ...
    print("Min of atomic UQ for H and C: ",hmin,cmin)
    print("Atomic UQ of molecules",np.shape(std_aen),np.shape(tspecies),np.shape(zeros),np.shape(sarray))
    print("masked ", hmask[0],cmask[0],std_aen[0])

    std_aen=std_aen-hshift-cshift #all atoms
    ...
    d=ma.masked_where(std_aen>cri_auq,std_aen)
    #d=ma.masked_where(std_aen>0.0,std_aen)
    
    newmask = d.mask
    
    newref=std_aen[newmask]
    print("Masked energy shape",np.shape(newref),np.shape(newmask))
    ...
    #3 error meV/atom
    #4 UQ (meV)
    #5 Mean of UQ (at each data) (meV)
    #6 MAX of UQ (at each data) (meV)
    #7 Criteria Mean+2*STD
    #9 
    for i in range(0,len(nref_enr)):
        out+=str(i)+" "+str(abs(nexp_enr[i][0]-nref_enr[i][0]))+" "+str(abs(nexp_enr[i][0]-nref_enr[i][0])/natoms*0.043*1000)+" "+str(hartree2kcalmol(std_en[i][0])/natoms*0.043*1000)+" "+str(hartree2kcalmol(nmean_auq[i][0])*0.043*1000)+" "+str(hartree2kcalmol(nmax_auq[i][0])*0.043*1000)+" "+str(hartree2kcalmol(ncri_auq[i][0])*0.043*1000)+" "+str(hartree2kcalmol(nmax_auq[i][0]-ncri_auq[i][0])*0.043*1000)+"\n"
    efile="emap_"+filehead+".data"
    fout = open(efile,'w')
    fout.write(out)
    fout.close()
