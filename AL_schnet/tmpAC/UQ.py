...
def UQSch(filehead,nmodels,sig,maxnum,datanum):
    #nmodels=3
    #Initial Data #
    cutnum = 1000
    ...
    coords,tforces,cells,energies,species,virial=ReadHDF5(filename,1)
    
    #Load Data
    test = MD17(os.path.join(dbname))    
    ...
    #tdata = torchani.data.load(filename,
    #                           additional_properties=('forces','cell')
    #).species_to_ind770512ices(species_order) #.subtract_self_energies(energy_shifter, species_order).species_to_indices(species_order)
    #tdata = tdata.collate(batch_size).cache()
    ...
    avg_en = np.empty(shape=[0,1])
    std_en = np.empty(shape=[0,1])    

    avg_f=np.empty(shape=[0,natoms,3]) 
    std_f=np.empty(shape=[0,natoms,3])           
    
    for count, batch in enumerate(test_loader):
        #for i in range(0,nmodels):
        #print(batch['_positions'].size())
        tcoord=batch['_positions']
        tcell=batch['_cell']        
        
        ref_coords = np.append(ref_coords,tcoord.cpu().detach().numpy(),axis=0)
        ref_cell = np.append(ref_cell,tcell.cpu().detach().numpy(),axis=0)
        ...
        for i in range(0,nmodels):
            ...
            #print(batch.keys())
            # apply model
            pred = emodel.models[i](batch)

            energy = pred[MD17.energy]
            force = pred[MD17.forces]            
            #print(energy.size())
            tenergy.append(energy.detach().cpu().numpy())
            tforces.append(force.detach().cpu().numpy())            
            
            #print(pred[MD17.energy].size())
            #print(pred[MD17.forces].size())            
            #print(batch['_atomic_numbers'].size())
            #print(batch['_positions'].size())            
        ...
        nenergy = np.array(tenergy)
        nforces = np.array(tforces)        
        avg_energy = np.mean(nenergy,axis=0)
        std_energy = np.std(nenergy,axis=0)

        avg_forces = np.mean(nforces,axis=0)
        std_forces = np.std(nforces,axis=0)        
        
        #print(np.shape(tenergy),avgenergy.size(),avgenergy)
        #Check dimension
        print(np.shape(nenergy),np.shape(avg_energy),np.shape(avg_energy),np.shape(nforces),np.shape(avg_forces))
        #print(avg_en[0],std_en[0])

        std_en = np.append(std_en,std_energy,axis=0)
        avg_en = np.append(avg_en,avg_energy,axis=0)

        std_f = np.append(std_f,std_forces,axis=0)
        avg_f = np.append(avg_f,avg_forces,axis=0)
    ...
    print("Ref coord and cell",np.shape(ref_coords),np.shape(ref_cell),np.shape(avg_en),np.shape(std_en),np.shape(avg_f))
    print("Values of avg and std: ",avg_en[0],std_en[0],np.shape(std_en))

    mean_uq = np.mean(std_en[:cutnum])
    std_uq = np.std(std_en[:cutnum])    
    cri_uq = mean_uq+std_uq*sig
    
    print("Mean and STD of UQ",mean_uq,std_uq,np.shape(mean_uq),np.shape(std_uq))
    #std_en=std_en.squeeze()
    ...
    selout=""
    torder=[]
    for i in range(0,len(avg_en)):
        tdist = Dist(ref_coords[i],9,16)
        pe = avg_en[i].item()*0.043
    ...
    dE = 0.043*1.0 # Energy cut for Total # of atoms, eV
    dr = 0.01
    ...
    for i in range(1,len(avg_en)):
        tdist = Dist(ref_coords[i],9,16)
        xd = abs(tdist-cdist)
        Ed = abs(avg_en[i]*0.043-cE)
    ...
    new_en=avg_en[newmask] #Configuration Selection
    new_coords=ref_coords[newmask.squeeze()] #Configuration Selection
    new_forces=avg_f[newmask.squeeze()] #Configuration Selection
    ...
    for i in range(0,len(std_en)):
        ...
        out+=str(i)+" "+str(avg_en[i][0])+" "+str((std_en[i][0]))+"\n"
    efile="emap_"+filehead+".data"
    fout = open(efile,'w')
    fout.write(out)
    fout.close()
    ...
    #SaveHDF5('selected.h5',new_coords,new_forces,new_en,species,new_cell,virial)
    SaveHDF5('selected.h5',new_coords[:maxnum],new_forces[:maxnum],new_en[:maxnum],species,new_cell[:maxnum],virial[:maxnum])    
    ...
    atoms=[]
    property_list=[]
    for i in range(0,len(ref_coords)):
        tcell = np.transpose(cells[i])
        mol = Atoms(systring,positions=coords[i],cell=tcell,pbc=1)        
        atoms.append(mol)
        property_list.append(
            {'energy':np.array([avg_en[i]],dtype=np.float32),'forces':np.array(avg_forces[i],dtype=np.float32)}
        )

    new_dataset = AtomsData('check.db', available_properties=['energy','forces'])
...
UQSch(dataname,nensem,sig,maxnum,1000)
