...
def post_force_callback(lmp, v, m):
    ...
    q = L.numpy.extract_atom("q", nelem=nlocal, dim=1)
    ...
    if(device_str=='cpu'):
        ...
        #cell
        cell = np.array([[boxhi[0]-boxlo[0],0,0],[xy,boxhi[1]-boxlo[1],0],[xz,yz,boxhi[2]-boxlo[2]]])        

        #ASE atoms arrays
        ats = Atoms(systring,positions=x,cell=cell,pbc=0)
        
        emodel.gvalues(ats)
        
        energy = np.mean(emodel.nenergy,axis=0)
        force = np.mean(emodel.nforces,axis=0)
        #stress =  torch.mean(emodel.nstress,dim=0) #no stress function yet
        
        #Need to further make the code work for MPI communications
        euq=np.std(emodel.nenergy)
        ...
        f[:][:] = force[:nlocal][:]*kcal2ev
        tarray[9]=energy.sum()*kcal2ev
        for i in range(0,3):
            for j in range(0,3):
                tarray[i*3+j]=0.0
